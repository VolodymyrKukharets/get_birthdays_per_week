...
def get_birthdays_per_week(users, today):
    ...
    for user in users:
        ...
        try:
            birthday_this_year = datetime.datetime(year=today.year, month=user["birthday"].month, day=user["birthday"].day)
        except:
            birthday_this_year = datetime.datetime(year=today.year, month=3, day=1)

        #if diff = 0 days we got [00:00:00], in this situation we got an error
        try:
